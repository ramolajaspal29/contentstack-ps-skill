---
name: contentstack-ps
description: Manage Contentstack via PowerShell on Windows — create, update, delete, publish, and query content types, entries, assets, taxonomies, releases, workflows, webhooks, environments, and more using Invoke-WebRequest against both the Content Management API (CMA) and Content Delivery API (CDA). Supporting reference files (Postman collections + Invoke-WebRequest docs) live in ~/.claude/skills/contentstack/.
---
...
All files listed below live in `~/.claude/skills/contentstack/`. Refer to them whenever you need details beyond what this skill's functions cover.

| File | Purpose |
|---|---|
| `Content Management API - Contentstack.postman_collection.json` | Complete CMA reference — every write endpoint (stacks, content types, entries, assets, releases, workflows, webhooks, bulk ops, tokens, roles, etc.) with exact URLs, HTTP methods, required headers, and sample request bodies |
| `Content Delivery API - Contentstack.postman_collection.json` | Complete CDA reference — every read endpoint for published content (entries, assets, taxonomies, sync, all query operators) with sample URLs |
| `Invoke-WebRequest.md` | Official Microsoft docs for the `Invoke-WebRequest` cmdlet (PowerShell 5.1) — covers all parameters including `-UseBasicParsing`, `-Method`, `-Headers`, `-Body`, `-InFile`, `-OutFile`, `-ContentType`, `-TimeoutSec`, and session handling |

### When to refer to these files

- **Postman JSONs** — when you need an endpoint not wrapped by a named function in this skill, or need to verify the exact request body shape, available query parameters, or a specific header requirement. Search by endpoint name (e.g. `"Atomic updates"`, `"Set Field Visibility Rule"`, `"Export a content type"`).
- **Invoke-WebRequest.md** — when you need to handle advanced HTTP scenarios: custom certificates, proxy settings, session cookies, file downloads (`-OutFile`), file uploads (`-InFile`), or timeout configuration.

### Quick search in PowerShell
```powershell
# Load both collections once per session
$skillDir = "$env:USERPROFILE\.claude\skills\contentstack"   # supporting docs folder
$cma = Get-Content "$skillDir\Content Management API - Contentstack.postman_collection.json" | ConvertFrom-Json
$cda = Get-Content "$skillDir\Content Delivery API - Contentstack.postman_collection.json" | ConvertFrom-Json
...
# Examples:
Find-CSEndpoint -Items $cma.item -Search "atomic"      # atomic update operations
Find-CSEndpoint -Items $cma.item -Search "visibility"  # field visibility rules
Find-CSEndpoint -Items $cda.item -Search "taxonomy"    # taxonomy CDA endpoints
```
